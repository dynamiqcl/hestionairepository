...
export async function analyzeReceiptImage(filePath: string) {
  try {
    ...
    // Si es un PDF, actualmente solo enviamos un mensaje indicando que no podemos procesarlo directamente
    // En un sistema más avanzado, se convertiría el PDF a imagen primero
    if (fileType === '.pdf') {
      return {
        success: false,
        message: 'PDF directo no soportado',
        extractedData: {
          date: new Date(),
          total: 0,
